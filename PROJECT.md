...
**발표용 이미지 동기화:** 발표 캡처는 저장소 안의 `발표용 이미지 캡처/` 폴더에 넣어야 GitHub·다른 PC로 넘어간다. 현재 이 폴더에서 Git에 추적되는 파일은 `대화첨부_42_image-b522681d-437c-4514-ab4b-c3ff34040a60.png`와 `대화첨부_이미지_목록.md`이다. 맥에서 01~41번 캡처가 로컬에만 있다면 같은 폴더에 PNG를 복사한 뒤 채팅에 `깃동기화`를 실행하면 된다. Cursor 대화 첨부/임시 assets 경로에만 있는 이미지는 저장소 밖이라 자동 동기화되지 않는다.
